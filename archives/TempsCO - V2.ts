...
var CONNECTIT: Map<string, Map<string, { time: number, needsTurnaround: boolean }>>;
...
function main(workbook: ExcelScript.Workbook) {
    ...
    let variants = createConnectionsAndVariants();
...
function calculateCompletePath(combination: string[], changeTime: number): { path: string[], totalDistance: number } {
    ...
    for (let i = 0; i < combination.length - 1; i++) {
        ...
        let segmentPath = dijkstra(CONNECTIT, segmentStart, segmentEnd, changeTime);
...
function calculatePathTime(path: string[], changeTime: number): number {
    ...
    for (let i = 0; i < path.length - 1; i++) {
        let from = path[i];
        let to = path[i + 1];
        let connection = CONNECTIT.get(from)?.get(to);
        if (connection) {
 ...
 * @param connecti2 - La carte des connexions entre les gares.
 * @param start - La gare de départ.
 * @param end - La gare d'arrivée.
 * @returns Le chemin le plus court.
 */
function dijkstra(connecti2: Map<string, Map<string, { time: number, needsTurnaround: boolean }>>, start: string, end: string, changeTime: number): string[] {
    let distances = new Map<string, number>();
    let previousNodes = new Map<string, string | null>();
    let unvisited = new Set<string>(CONNECTIT.keys());
    let path: string[] = [];
    ...
    while (unvisited.size > 0) {
        ...
        for (let [neighbor, { time, needsTurnaround }] of CONNECTIT.get(currentNode) || []) {
...
function generateCombinations(start: string, end: string, via: string[], variants: Map<string, string[]>): string[][] {
    ...
    let allCombinations: string[][] = routes.flatMap((route: string[]) => expandPermutations(route, variants));
...
function createConnectionsAndVariants(): {variants: Map<string, string[]>} {
    CONNECTIT = new Map<string, Map<string, { time: number, needsTurnaround: boolean }>>();
    ...
    for (let row of data) {
        ...
        if (!CONNECTIT.has(from)) {
            CONNECTIT.set(from, new Map<string, { time: number, needsTurnaround: boolean }>());
        }
        CONNECTIT.get(from).set(to, { time, needsTurnaround });
