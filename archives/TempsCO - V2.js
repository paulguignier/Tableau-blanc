...
function calculatePathTime(path, changeTime) {
    ...
    for (let i = 0; i < path.length - 1; i++) {
        let from = path[i];
        let to = path[i + 1];
        let connection = (_a = CONNECTIT.get(from)) === null || _a === void 0 ? void 0 : _a.get(to);
        if (connection) {
 ...
 */
function dijkstra(connecti2, start, end, changeTime) {
    let distances = new Map();
    let previousNodes = new Map();
    let unvisited = new Set(CONNECTIT.keys());
    let path = [];
