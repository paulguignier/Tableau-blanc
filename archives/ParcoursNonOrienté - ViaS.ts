function main(workbook: ExcelScript.Workbook) {
    ...
    let viaRaw = sheet.getRange("B3").getValue() as string;
...
function findOptimalPath(graph: { [key: string]: string[] }, start: string, vias: string[], end: string): string[] | null {
...
function findCompletePath(graph: { [key: string]: string[] }, stations: string[]): string[] | null {
    let path: string[] = [];
...
function findShortestPath(graph: { [key: string]: string[] }, start: string, end: string): string[] | null {
    let queue: { station: string; path: string[] }[] = [{ station: start, path: [start] }];
