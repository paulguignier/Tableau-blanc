...
/* Carte des gares et leurs coordonnées. */
var STATIONS: Record<string, Station> = {};
/* La carte des connexions entre les gares, incluant le temps de trajet et l'information sur le besoin de changement de sens. */
var CONNECTIONS: Map<string, Map<string, Connection>> = {};
/* La carte des variantes de parcours. */
...
function main(workbook: ExcelScript.Workbook) {
    ...
    // createConnectionsAndVariants();
    // printStations(sheet, "F1");
    ...
    let allCombinations = generateCombinations(start, end, via);

    // Trouver le chemin le plus court parmi toutes les combinaisons
    let shortestPath = findShortestPath(allCombinations, changeTime);
 ...
 */
function findShortestPath(allCombinations: string[][], changeTime: number): { path: string[], totalDistance: number } | null {
    ...
    for (let combination of allCombinations) {
        ...
        let { path, totalDistance } = calculateCompletePath(combination, changeTime);
 ...
 */
function calculateCompletePath(combination: string[], changeTime: number): { path: string[], totalDistance: number } {
    ...
    for (let i = 0; i < combination.length - 1; i++) {
        ...
        let segmentPath = dijkstra(segmentStart, segmentEnd, changeTime);
        ...
        let segmentDistance = calculatePathTime(segmentPath, changeTime);
 ...
 */
function calculatePathTime(path: string[], changeTime: number): number {
    let totalTime  = 0;
...
function generateCombinations(start: string, end: string, via: string[]): string[][] {
    ...
    let routes: string[][] = viaPermutations.map(permutation => [start, ...permutation, end]);

    // Étendre chaque route pour inclure toutes les variantes possibles
    let allCombinations: string[][] = routes.flatMap((route: string[]) => expandPermutations(route));
...
/**
 * Affiche un tableau à partir d'un tableau de données dans une feuille de calcul Excel.
 * @param tableData - Le tableau de données à afficher.
 * @param sheet - La feuille de calcul où afficher le tableau.
 * @param startCell - La cellule où commencer à afficher le tableau.
 * @param tableName - Le nom du tableau à afficher.
 */
function printTable(tableData: string[][], sheet: ExcelScript.Worksheet, startCell: ExcelScript.Range, tableName: string): void {
 ...
 * @param {ExcelScript.Worksheet} sheet - La feuille de calcul où afficher le tableau.
 * @param {string} startCell - La cellule de départ du tableau.
 */
function printStations(sheet: ExcelScript.Worksheet, startCell: string) {
    /**
     * Affiche les stations dans un tableau.
     * @param {ExcelScript.Worksheet} sheet - La feuille de calcul où afficher le tableau.
     * @param {string} startCell - La cellule de départ du tableau.
    ...
    // Vérifier que la variable globale STATIONS est remplie
    if (Object.keys(STATIONS).length === 0) {
        loadStations();
    }

    // Préparer les en-têtes du tableau
    const headers = [["Abréviation", "Nom", "Variantes", "Changements de parité"]];

    // Préparer les données des stations
    ...
    // Imprimer le tableau
    printTable(tableData, sheet, startCell, "StationsTable");
}
...
class Connection {
     ...
     * @param {number} changeParity - Indique le changement de parité
     */
    constructor(from: string, to: string, time: number, needsTurnaround: boolean) {
        ...
        let fromParity: number = parseInt(from.split('_')[1], 10) % 2;
        let toParity: number = parseInt(to.split('_')[1], 10) % 2;
...
/**
 * Crée les connexions entre les gares et les variantes de ces gares.
 ...
 */
function createConnectionsAndVariants() {
    CONNECTIONS = new Map<string, Map<string, Connection>>();
    ...
    let sheet = WORKBOOK.getWorksheet(SHEET_CONNECTIONS);
    if (!sheet) {
        console.log("La feuille " + SHEET_CONNECTIONS + " n'existe pas !");
        return;
    }
    const table = sheet.getTable(TABLE_CONNECTIONS);
    if (!table) {
        console.log("Le tableau " + TABLE_CONNECTIONS + " n'existe pas !");
        return;
    }
    let data = table.getRange().getValues();

    for (let row of data) {
        let from = row[CONNECTIONS_COL_FROM] as string;
        let to = row[CONNECTIONS_COL_TO] as string;
        let time = row[CONNECTIONS_COL_TIME] as number;
        let needsTurnaround = row[CONNECTIONS_COL_NEEDS_TURNAROUND] as boolean;
...
/**
 * Ajoute une variante de gare au système.
 * Si la gare de base n'existe pas dans les données actuelles des gares, elle est créée avec un nom par défaut.
 * La variante est ajoutée à la liste des variantes de la gare et à la Map des variantes globales.
 * @param variant - La variante de gare à ajouter, sous la forme 'nom_gare_suffixe'.
 */
function addVariants(variant: string) {
    let baseVariant = variant.split('_')[0];
    if (!STATIONS[baseVariant]) {
        STATIONS[baseVariant] = new Station(baseVariant, "Gare inconnue : " + baseVariant, []);
    }
    STATIONS[baseVariant].variants.push(variant);
    if (!VARIANTS.has(baseVariant)) {
        VARIANTS.set(baseVariant, []);
    }
    if (!VARIANTS.get(baseVariant)!.includes(variant)) {
        VARIANTS.get(baseVariant)!.push(variant);
    }
}
