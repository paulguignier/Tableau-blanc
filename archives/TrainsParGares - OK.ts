...
function main(workbook: ExcelScript.Workbook) {
    WORKBOOK = workbook;
    let sheet: ExcelScript.Worksheet = WORKBOOK.getWorksheet(SHEET_TB) as ExcelScript.Worksheet;
    let gare: string = sheet.getRange(TB_CELL_GARE).getValue() as string;
    let jour: number = (typeof sheet.getRange(TB_CELL_JOUR).getValue() === 'number' ? sheet.getRange(TB_CELL_JOUR).getValue() : 1) as number;
    let heure: number = (typeof sheet.getRange(TB_CELL_HEURE).getValue() === 'number' ? sheet.getRange(TB_CELL_HEURE).getValue() : 0) as number;
    let arrivee_depart: number = (typeof sheet.getRange(TB_CELL_ARRIVEE_DEPART).getValue() === 'number' ? sheet.getRange(TB_CELL_ARRIVEE_DEPART).getValue() : 0) as number;

    let range: ExcelScript.Range = sheet.getRange(TB_COLS_TRAINPARGARE); // Sélectionner les colonnes entières
    range.clear();
    range.getCell(0, 0).setValue("Train");
    range.getCell(0, 1).setValue("Heure");
    range.getCell(0, 2).setValue("Réut");

    let trains: Record<string, Train> = chargerTrains();
    chargerArrets(trains);
    let trainsParGare: Train[] = getTrainsByGare(gare, jour, heure, trains);
 ...
 */
function getTrainsByGare(gare: string, jour: number, heure: number, trains: Record<string, Train>): Train[] {
    let trainsByGare: Train[] = [];

    Object.keys(trains).forEach((trainKey) => {
        let train: Train = trains[trainKey];
    ...
    trainsByGare.sort((a, b) => {
        let heureA: number = a.arrets[gare].getHeure();
        let heureB: number = b.arrets[gare].getHeure();
...
function writeTrains(range: ExcelScript.Range, gare: string, trains: Train[]) {
    ...
    Object.keys(trains).forEach((trainKey) => {
        let train: Train = trains[trainKey];
        let arret: Arret = train.arrets[gare];
        if (arret) {
...
class Train {
     ...
     */
    constructor(numero: number, sens: number, jour: number, codeMission: string, heureDepart: number, gareDepart: string, heureArrivee: number, gareArrivee: string, garesVia: string, reutilisation: string) {
        this.numero = numero;
        this.sens = sens;
        this.jour = jour;
 ...
 */
function chargerTrains(): Record<string, Train> {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let heureDepart = data[i][TRAINS_COL_HEURE_DEPART] as number;
        ...
        let garesVia = data[i][TRAINS_COL_GARES_VIA] as string;
        let reutilisation = data[i][TRAINS_COL_REUTILISATION] as string;
...
class Arret {
     ...
     */
    constructor(parite: number, gare: string, heureArrivee: number, heureDepart: number, voie: string) {
        this.parite = parite;
        this.gare = gare;
        this.heureArrivee = heureArrivee;
        this.heureDepart = heureDepart;
...
function chargerArrets(trains: Record<string, Train>) {
...
class Station {
     ...
     */
    constructor(abbreviation: string, name: string, reversedParity: boolean, variants: string[] = []) {
 ...
 */
function chargerGares(): Record<string, Gare> {
    ...
    for (let i = 1; i < data.length; i++) {
        let abreviation = data[i][GARES_COL_ABRV] as string;
        let nom = data[i][GARES_COL_NOM] as string;;
        let pariteInversee = data[i][GARES_COL_PARITE_INVERSEE] as boolean;
