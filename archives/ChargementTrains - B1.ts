 ...
 */
function getSheetOrFail(sheetName: string, failOnError = true): ExcelScript.Worksheet | null {
 ...
 */
function getTableOrFail(sheetName: string, tableName: string, failOnError: boolean = true): ExcelScript.Table | null {
 ...
 */
function getDataFromTable(sheetName: string, tableName: string, failOnError: boolean = true): (string | number | boolean)[][] {
 ...
 */ 
function printTable(headers: string[][], data: (string | number)[][], sheetName: string, tableName: string, startCell: string = "A1", failOnError: boolean = true): ExcelScript.Table | null {
...
function renameWith4Figures(trainNumber: string): string {
    ...
    if (!PARAM.trains4FiguresRegex) load4FiguresTrainsRegex(); // Charge si non encore fait
...
class Train {
    ...
    constructor(number: number, days: string, missionCode: string, departureTime: number, departureStation: string, arrivalTime: number, arrivalStation: string, viaStations: string) {
        this.number = number;
        this.days = days;
...
function loadTrains(days: string = "JW", trains: string = ""): Map<string, Train> {
    ...
    const trainDaysCache = new Map<string, { hasCommonDays: boolean, trainDaysTable: number[] }>();
    ...
    for (let row of data.slice(1)) {
        ...
        const days = row[TRAINS_COL_DAY] as string;
...
class Reuse {
    ...
    constructor(number: number, train: Train, day: number, unit1: string = "", unit2: string = "", previous1: string = "", previous2: string = "", reuse1: string = "", reuse2: string = "") {
        this.number = number;
        this.train = train;
        this.day = day;
        this.unit1 = unit1;
...
class Stop {
    ...
    constructor(stationName: string, parity: number = -1, arrivalTime: number = 0, departureTime: number = 0, passageTime: number = 0, track: string = "", changeNumber: number = 0) {
...
function loadStops() {
    ...
    const stopDaysCache = new Map<string, number[]>();
...
class Station {
    ...
    constructor(abbreviation: string, name: string, variants_stations: string[], odd_reversal: boolean, even_reversal: boolean, lineC_reverse_direction: boolean) {
...
function loadStations(erase: boolean = false) {
    ...
    if (Object.keys(STATIONS).length > 0) {
        ...
        STATIONS = new Map<string, Map<string, Connection>>();
    }
    ...
    for (let row of data.slice(1)) {
        ...
        // Extraction des valeurs
        let abbreviation = row[STATIONS_COL_ABBR] as string;
        let name = row[STATIONS_COL_NAME] as string;
        let variants_stations = (row[STATIONS_COL_VARIANTS_STATIONS] as string).split(";");
        let odd_reversal = row[STATIONS_COL_REVERSAL].indexOf('I') >= 0 as boolean;
        let even_reversal = row[STATIONS_COL_REVERSAL].indexOf('P') >= 0 as boolean;
        let lineC_reverse_direction = row[STATIONS_COL_LINEC_REVERSE_DIRECTION] === 1 as boolean;
