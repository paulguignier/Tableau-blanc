function main(workbook: ExcelScript.Workbook) {
    ...
    let debugConnections = Array.from(connections.entries()).map(([from, edges]) =>
        `${from} -> ${Array.from(edges.entries()).map(([to, time]) => `${to} (${time}min)`).join(", ")}`
    );
    ...
    for (let combination of allCombinations) {
        ...
        let variant:string = combination.join(' -> ');
