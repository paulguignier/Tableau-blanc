...
function main(workbook) {
    ...
    // Trouver tous les variants possibles pour les gares de départ et d'arrivée
    let startVariants = getAllVariants(start, variants);
    let endVariants = getAllVariants(end, variants);
    // Affichage des connexions dans la colonne F
    let debugConnections = Array.from(connections.entries()).map(([from, edges]) => `${from} -> ${Array.from(edges.entries()).map(([to, time]) => `${to} (${time}min)`).join(", ")}`);
    if (debugConnections.length > 0) {
        sheet.getRange("F5:F" + (4 + debugConnections.length)).setValues(debugConnections.map(val => [val]));
    }
    // Dijkstra pour chaque variante
    let columnIndex = 7; // Commence à la colonne G (index 6)
    allCombinations = generateCombinations(start, end, via, variants);
    for (let combination of allCombinations) {
        let columnLetter = String.fromCharCode(64 + columnIndex); // Convertir le numéro de colonne en lettre
        let variant = combination.join(' -> ');
        sheet.getRange(`${columnLetter}4`).setValue(variant);
        // Calculer le chemin complet pour la combinaison actuelle
        let { path, totalDistance } = calculateCompletePath(connections, combination);
        if (path.length > 0) {
            // Afficher le chemin et la distance totale
            sheet.getRange(`${columnLetter}5`).setValue(`Distance totale : ${totalDistance} min`);
            for (let i = 0; i < path.length; i++) {
                sheet.getRange(`${columnLetter}${i + 6}`).setValue(path[i]);
            }
        }
        else {
            sheet.getRange(`${columnLetter}5`).setValue("Aucun chemin trouvé");
        }
        columnIndex++;
    }
}
 ...
 */
function calculateCompletePath(connections, combination) {
    let completePath = [];
    let totalDistance = 0;
 ...
 */
function calculatePathDistance(connections, path) {
    var _a, _b;
    let distance = 0;
    for (let i = 0; i < path.length - 1; i++) {
        let from = path[i];
        let to = path[i + 1];
        distance += (_b = (_a = connections.get(from)) === null || _a === void 0 ? void 0 : _a.get(to)) !== null && _b !== void 0 ? _b : 0;
    }
    return distance;
}
/**
 ...
 */
function dijkstra(connections, start, end) {
