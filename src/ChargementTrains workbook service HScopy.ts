...
/* Service d'accès au classeur Excel */
var WORKBOOK_SERVICE: WorkbookService;
...
function main(workbook: ExcelScript.Workbook) {
    ...
    // Initialisation des variables globales : classeur, consoles
    WORKBOOK = workbook;
    CONSOLE_INFO = console;
    CONSOLE_WARN = console;
    CONSOLE_DEBUG = console;

    // Programme en mode déboggage en utilisant la fonction test
...
function tests(debugMode = false): boolean {
    ...
    //     "Col2"
    // ]];
    // const data = WorkbookService.getDataFromTable(SHEET, TABLE);
    // CONSOLE_INFO.log(data);
    // const data2 = [
    //     ["A", "B"],
    //     ["C", "D"]
    // ];
    // const table = WorkbookService.printTable(HEADERS, data2, SHEET, TABLE, "A2", false);
    // CONSOLE_INFO.log(WorkbookService.checkCellName("A1"));
    // CONSOLE_INFO.log(WorkbookService.checkCellName("1A")); // Doit aboutir un une erreur
...
/**
 * Service centralisant les opérations sur le classeur Excel.
 * Responsabilité : accès aux feuilles, tableaux, lecture/écriture de données.
 */
class WorkbookService {
     ...
     */
    static getSheetOrFail(
        sheetName: string,
        failOnError: boolean = true
    ): ExcelScript.Worksheet | null {
        // const w : ExcelScript.Workbook = WORKBOOK;
        // const sheet = w.getActiveWorksheet();
     ...
     *  lance une exception si le tableau n'existe pas. Si faux, renvoie un tableau vide.
     * @returns {(string | number | boolean)[][]} Données du tableau Excel
     *  correspondant au nom donné, ou tableau vide si il n'existe pas.
     */
    static getDataFromTable(
        sheetName: string,
        tableName: string,
        failOnError: boolean = true
    ): (string | number | boolean)[][] {
        const table = WorkbookService.getTableOrFail(sheetName, tableName, failOnError);
        if (!table) return [];
     ...
     * Si elle est valide, la renvoie telle quelle en majuscules.
     ...
     */
    static checkCellName(cellName: string, failOnError: boolean = true): string {
        // Convertit cellName en majuscules pour éviter les problèmes de casse
     ...
     * @param {(string | number | boolean)[][]} data Données du tableau.
    ...
    static printTable(
        ...
        const startRange = sheet.getRange(this.checkCellName(startCell, failOnError));
        ...
        CONSOLE_INFO.log(`Le tableau "${tableName}" a été créé avec succès dans la feuille "${sheetName}".`);

        return table;
    }

    /**
     * Récupère la feuille active du classeur.
     * @returns {ExcelScript.Worksheet} Feuille active
     */
    static getActiveWorksheet(): ExcelScript.Worksheet {
...
function loadParams(erase: boolean = false) {
    ...
    const data = WorkbookService.getDataFromTable(PARAM_SHEET, PARAM_TABLE);
...
function loadWTrainsRegex() {
    const data = WorkbookService.getDataFromTable(W_SHEET, W_TABLE);
...
function load4DigitsAbreviatedTrainsRegex() {
    const data = WorkbookService.getDataFromTable(TRAINS_4DIGITS_SHEET, TRAINS_4DIGITS_TABLE);
...
function loadParity() {
    ...
    const data = WorkbookService.getDataFromTable(PARITY_SHEET, PARITY_TABLE);
...
function loadStations(erase: boolean = false) {
    ...
    const data = WorkbookService.getDataFromTable(STATIONS_SHEET, STATIONS_TABLE);
...
function printStations(sheetName: string, tableName: string, startCell: string = "A1"): void {
    ...
    // Imprime le tableau
    WorkbookService.printTable(STATIONS_HEADERS, data, sheetName, tableName, startCell);
}
...
function loadConnections(erase: boolean = false) {
    ...
    loadStations();
    const data = WorkbookService.getDataFromTable(CONNECTIONS_SHEET, CONNECTIONS_TABLE);
...
function printConnections(sheetName: string, tableName: string, startCell: string = "A1"): void {
    ...
    // Imprime le tableau
    const table = WorkbookService.printTable(CONNECTIONS_HEADERS, data, sheetName, tableName, startCell);
